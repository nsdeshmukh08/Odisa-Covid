...
class Enterprises extends Component {
    ...
    onTableDataClick = (tableData, index) => {
        ...
        let { label, value } = data.formList[index]
    ...
    render() {
        ...
        const { locationType,search } = this.state
        ...
        // let masterList = this.getFilteredTableData().length ? this.getFilteredTableData()[0] : []

        // let dynamicRow = [
        //     ...chartListData.columns,
        //     // ...this.getFilteredTableData().length ? this.getFilteredTableData()[0].masterList.map(data => ({
        //     //     header: data.label,
        //     //     Cell : ({masterList}) => masterList.find(cellValue => data.label === cellValue.label).count 
        //     // })) : []
        ...
        console.log(chartListData.columns,"columns")
            ...
            <Fragment>
                {
                    locationType <= 2 //CHECKING WHETHER IT IS NOT PANCHAYAT VIEW
                        ? <div className="bg-white position-relative">
                            <StackedBarChart graphData={data.graphData} locationType={locationType}/>
                            {
