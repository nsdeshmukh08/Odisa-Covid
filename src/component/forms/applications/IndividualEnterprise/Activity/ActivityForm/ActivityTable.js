    ...
    <div className="activity-list">
      ...
      {props.data.map((data, index) => (
        <Row className="custom-select-head">
          ...
          <Col
            md="2"
            className="p-0 active-select position-relative remove-label"
          >
            <FormInput
              type="select"
      ...
      <Row className="text-right">
        <Col>
          ...
          <Button color="link" onClick={() => props.onAddOrRemoveActivity()}>
            + {themeData.addActivity}
          </Button>
        </Col>
      </Row>
      <Row className="acivity-head total mt-3">
        <Col md="8">
          <p className="fw-700"> {themeData.totalAmmountRequired}</p>
        </Col>
        <Col md="4">
          <p className="fw-700">
            Rs{' '}
            {props.data
              .filter((data) => data.amtReq)
              .reduce((acc, value) => {
