...
export class BankDetailsClass extends Component {
    ...
    //HANDLE CHANGE
    onChange = (name, value) => {
        let { pcFormBankDetails, errors } = this.state
        pcFormBankDetails[name] = value
        errors[name] = undefined
        this.setState({ pcFormBankDetails, errors })
    }
    onSubmit = (e) => {
        e.preventDefault()
        let { pcFormBankDetails,cancelToken } = this.state
        const notValid = validate(pcFormBankDetails, inputValidations);
        if (notValid) {
            this.setState({
                errors: notValid
            })
            ...
            pcFormBankDetails['formId']=localStorage.getItem('createAppformId')
            // this.props.history.push((currentSection + 1).toString())
            this.props.updateForm(
                {data:pcFormBankDetails,stage},
                cancelToken.token
            )
        }
    }
    ...
    render() {
        ...
        const { errors, pcFormBankDetails ,loading} = this.state
                        ...
                        <Row className="w-100 d-flex justify-content-end align-items-center m-0 ">
                            <Col>
                                <Link to={(currentSection - 1).toString()}>
                                    <Button type="button" outline color="lighGrey-2 w-100 border-none mb-3" className="fw-600">Previous</Button>
                                </Link>
                            </Col>
                            <Col>
                                {/* <Link to={(currentSection + 1).toString()}> */}
