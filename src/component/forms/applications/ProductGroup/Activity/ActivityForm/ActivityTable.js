...
const ActivityTable = (props) => {
    let { pcMasterData } = useSelector( state => state.common)
    return (
        <div className="activity-list">
                ...
                props.data && props.data.map((data, index) => (
                    <Row className="custom-select-head">
                        <Col className="remove-label">
                            <FormInput
                                type="only-text"
                                placeholder={`Name of the Activity ${index + 1}`}
                                name="activityName"
                        ...
                        <FormInput
                                type="select"
                                options={pcMasterData.activityData}
            ...
            <Row className="text-right">
                <Col> <Button color="link" onClick={() => props.onAddOrRemoveActivity()}>+ Add activity</Button></Col>
            </Row>
            <Row className="acivity-head total mt-3">
                <Col md="8"><p className="fw-700"> Total Amount Required</p></Col>
                <Col md="4"><p className="fw-700">Rs {
                    props.data && props.data.filter(data => data.amtReq).reduce((acc, value) => {
