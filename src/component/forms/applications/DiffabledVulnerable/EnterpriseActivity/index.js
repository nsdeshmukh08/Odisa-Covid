...
class EnterpriseActivityClass extends Component {
  ...
  //HANDLE CHANGE
  onChange = (name, value) => {
    let { iaEnterprise, errors } = this.state;
    iaEnterprise[name] = value;
    errors[name] = undefined;
    this.setState({ iaEnterprise, errors });
  };

  onSubmit = (e) => {
    e.preventDefault();

    let { iaEnterprise, cancelToken } = this.state;
    let validation = {
      ...inputValidations,
    };
    const notValid = validate(iaEnterprise, validation);
    ...
    iaEnterprise['formId'] = localStorage.getItem('createAppformId');
    // this.props.history.push((currentSection + 1).toString());
    this.props.updateForm({ data: iaEnterprise, stage }, cancelToken.token);
    // }
  };
